...
const FORGEBENCH_WORKER_SANDBOX_PREPARE_REQUEST_SCHEMA = "outilsia.forgebench_worker_sandbox_prepare_request.v1";
...
const els = {
  ...
  forgeBenchSandboxState: $("forgeBenchSandboxState"),
  forgeBenchSandboxBox: $("forgeBenchSandboxBox"),
  ...
  forgeBenchCandidateState: $("forgeBenchCandidateState"),
  forgeBenchCandidateModel: $("forgeBenchCandidateModel"),
...
function forgeBenchStackLabel(value) {
  return ({
    ...
    "ollama-local": "Modèle Ollama local"
  })[value] || value;
}
...
function renderForgeBenchVaultPanel() {
  ...
  els.forgeBenchVaultBox.innerHTML = `
    ...
    <small>Contenu non renvoyé · stockage local non chiffré · non monté dans le pilote · agents candidats bloqués</small>
  `;
}
...
function forgeBenchSandboxReceipt(status = state.forgeBenchWorkerSandbox) {
  const receipt = status?.receipt;
  const digest = receipt?.integrity?.digest;
  const workspaces = Number(receipt?.workspaces_total || 0);
  const stacks = Number(receipt?.candidate_stacks_total || 0);
  const seeds = Number(receipt?.public_seeds_total || 0);
  if (
    receipt?.schema !== FORGEBENCH_WORKER_SANDBOX_RECEIPT_SCHEMA
    || !/^fbsb-[a-z0-9_-]{8,64}$/.test(String(receipt?.batch_id || ""))
    || !/^[a-f0-9]{64}$/i.test(String(receipt?.experiment_digest || ""))
    || !/^[a-f0-9]{64}$/i.test(String(receipt?.protocol_digest || ""))
    || !/^[a-f0-9]{64}$/i.test(String(receipt?.starter_bundle_sha256 || ""))
    || !/^[a-f0-9]{64}$/i.test(String(receipt?.batch_digest || ""))
    || !/^[a-f0-9]{64}$/i.test(String(digest || ""))
    || stacks < 2 || stacks > 4 || seeds < 1 || seeds > 3 || workspaces !== stacks * seeds
    || receipt?.paths_returned !== false
    || receipt?.security?.fresh_workspace_per_run !== true
    || receipt?.security?.workspace_outside_source_repository !== true
    || receipt?.security?.starter_digest_verified !== true
    || receipt?.security?.hidden_suite_material_copied !== false
    || receipt?.security?.credentials_copied !== false
    || receipt?.security?.source_repository_mounted !== false
    || receipt?.security?.process_isolation_enforced !== false
    || receipt?.security?.network_isolation_enforced !== false
    || receipt?.security?.hidden_suite_access_blocked !== false
    || receipt?.execution?.worker_started !== false
    || receipt?.execution?.command_executed !== false
    || receipt?.readiness?.workspace_batch_prepared !== true
    || receipt?.readiness?.worker_execution_ready !== false
    || receipt?.readiness?.scientific_eligible !== false
  ) return null;
  const blockers = Array.isArray(receipt?.readiness?.blockers) ? receipt.readiness.blockers : [];
  for (const blocker of [
    ...
    if (!blockers.includes(blocker)) return null;
  }
  return receipt;
}

function forgeBenchSandboxMatchesExperiment(receipt = forgeBenchSandboxReceipt()) {
  const experiment = state.forgeBench?.experiment;
  return Boolean(
    receipt
    && experiment
    && receipt.experiment_id === experiment.experiment_id
    && receipt.experiment_digest === experiment.integrity?.digest
    && receipt.protocol_digest === experiment.protocol_digest
  );
}

function renderForgeBenchSandboxPanel() {
  if (!els.forgeBenchSandboxBox) return;
  const receipt = forgeBenchSandboxReceipt();
  const experiment = state.forgeBench?.experiment;
  const canPrepare = experiment?.readiness?.protocol_ready === true;
  const canClear = Boolean(receipt || forgeBenchSandboxError || state.forgeBenchWorkerSandbox?.exists);
  els.prepareForgeBenchSandboxBtn.disabled = !invoke || forgeBenchSandboxBusy || !canPrepare;
  els.refreshForgeBenchSandboxBtn.disabled = !invoke || forgeBenchSandboxBusy;
  els.clearForgeBenchSandboxBtn.disabled = !invoke || forgeBenchSandboxBusy || !canClear;
  if (!invoke && !state.forgeBenchWorkerSandbox?.test_mode) {
    els.forgeBenchSandboxState.textContent = "app native requise";
    els.forgeBenchSandboxBox.className = "forgebench-sandbox-box empty";
    els.forgeBenchSandboxBox.textContent = "La matérialisation des workspaces est disponible uniquement dans l'application Windows/Linux.";
    return;
  }
  if (forgeBenchSandboxBusy) {
    els.forgeBenchSandboxState.textContent = "préparation locale";
    els.forgeBenchSandboxBox.className = "forgebench-sandbox-box empty";
    els.forgeBenchSandboxBox.textContent = "Copie et vérification du starter public. Aucun worker, shell, réseau ou test caché n'est lancé.";
    return;
  }
  if (forgeBenchSandboxError) {
    els.forgeBenchSandboxState.textContent = "sandbox refusé";
    els.forgeBenchSandboxBox.className = "forgebench-sandbox-box empty";
    els.forgeBenchSandboxBox.innerHTML = `<strong>Espaces non fiables</strong><span>${escapeHtml(forgeBenchSandboxError)}</span>`;
    return;
  }
  if (!receipt) {
    els.forgeBenchSandboxState.textContent = canPrepare ? "prêts à matérialiser" : "préflight requis";
    els.forgeBenchSandboxBox.className = "forgebench-sandbox-box empty";
    els.forgeBenchSandboxBox.textContent = canPrepare
      ? "Crée un espace neuf par stack et seed public, hors du dépôt source, avec le starter embarqué vérifié."
      : "Prépare d'abord une expérience ForgeBench dont le protocole est prêt.";
    return;
  }
  const current = forgeBenchSandboxMatchesExperiment(receipt);
  const digest = receipt.integrity?.digest || "";
  els.forgeBenchSandboxState.textContent = current ? "workspaces vérifiés" : "batch ancien";
  els.forgeBenchSandboxBox.className = `forgebench-sandbox-box${current ? "" : " empty"}`;
  els.forgeBenchSandboxBox.innerHTML = `
    <strong>${escapeHtml(receipt.workspaces_total)} espaces frais ${current ? "liés au préflight" : "à remplacer"}</strong>
    <span>${escapeHtml(receipt.candidate_stacks_total)} stacks × ${escapeHtml(receipt.public_seeds_total)} seeds publics · starter SHA-256 vérifié</span>
    <small>Aucun chemin exposé · aucun worker lancé · processus, réseau et accès au vault non isolés · SHA-256 ${escapeHtml(digest ? `${digest.slice(0, 12)}…${digest.slice(-8)}` : "absente")}</small>
  `;
}

async function loadForgeBenchSandbox(silent = false) {
  if (!invoke || forgeBenchSandboxBusy) return null;
  forgeBenchSandboxBusy = true;
  forgeBenchSandboxError = "";
  if (!silent) setStatus("Vérification des workspaces ForgeBench...");
  renderForgeBenchSandboxPanel();
  try {
    const status = await invoke("get_forgebench_worker_sandbox_status");
    if (
      status?.schema !== FORGEBENCH_WORKER_SANDBOX_STATUS_SCHEMA
      || status?.paths_returned !== false
      || status?.hidden_suite_contents_returned !== false
      || status?.worker_started !== false
      || status?.command_executed !== false
    ) throw new Error("statut natif du sandbox non conforme");
    if (status.exists && !forgeBenchSandboxReceipt(status)) throw new Error("reçu de sandbox invalide");
    state.forgeBenchWorkerSandbox = status;
    if (!silent) setStatus(status.exists ? "Workspaces ForgeBench vérifiés" : "Aucun workspace ForgeBench", status.exists ? "ok" : "warn");
    return status;
  } catch (error) {
    state.forgeBenchWorkerSandbox = null;
    forgeBenchSandboxError = String(error || "Sandbox ForgeBench illisible");
    if (!silent) setStatus(`Sandbox ForgeBench : ${forgeBenchSandboxError}`, "error");
    return null;
  } finally {
...
async function prepareForgeBenchSandbox() {
  const experiment = state.forgeBench?.experiment;
  if (!invoke || forgeBenchSandboxBusy || experiment?.readiness?.protocol_ready !== true) return null;
  const replace = Boolean(forgeBenchSandboxReceipt());
  if (replace && !window.confirm("Remplacer le batch de workspaces ForgeBench ? Les espaces actuels seront supprimés.")) return null;
  forgeBenchSandboxBusy = true;
  forgeBenchSandboxError = "";
  renderForgeBenchSandboxPanel();
  setStatus(replace ? "Remplacement des workspaces ForgeBench..." : "Préparation des workspaces ForgeBench...");
  try {
    const result = await invoke("prepare_forgebench_worker_sandbox", {
      request: {
        schema: FORGEBENCH_WORKER_SANDBOX_PREPARE_REQUEST_SCHEMA,
        forgebench_result: state.forgeBench,
        replace_existing: replace
      }
    });
    if (
      result?.schema !== FORGEBENCH_WORKER_SANDBOX_PREPARE_RESULT_SCHEMA
      || result?.prepared !== true
      || result?.worker_started !== false
      || result?.command_executed !== false
      || result?.paths_returned !== false
    ) throw new Error("réponse native de préparation non conforme");
    const status = {
      schema: FORGEBENCH_WORKER_SANDBOX_STATUS_SCHEMA,
      exists: true,
      receipt: result.receipt,
    ...
    if (!forgeBenchSandboxReceipt(status)) throw new Error("reçu natif du sandbox invalide");
    state.forgeBenchWorkerSandbox = status;
    state.forgeBenchReferencePilot = null;
    state.forgeBenchOllamaCandidate = null;
    clearWorkstackArena(true);
    ...
    forgeBenchSandboxError = String(error || "Préparation du sandbox impossible");
    setStatus(`Sandbox ForgeBench : ${forgeBenchSandboxError}`, "error");
    return null;
  } finally {
    forgeBenchSandboxBusy = false;
    renderForgeBenchSandboxPanel();
...
async function clearForgeBenchSandbox() {
  if (!invoke || forgeBenchSandboxBusy || (!forgeBenchSandboxReceipt() && !forgeBenchSandboxError && !state.forgeBenchWorkerSandbox?.exists)) return;
  if (!window.confirm("Effacer tous les workspaces ForgeBench préparés ?")) return;
  forgeBenchSandboxBusy = true;
  forgeBenchSandboxError = "";
  renderForgeBenchSandboxPanel();
  try {
    const status = await invoke("clear_forgebench_worker_sandbox");
    if (status?.schema !== FORGEBENCH_WORKER_SANDBOX_STATUS_SCHEMA || status?.exists !== false) throw new Error("suppression native incomplète");
    state.forgeBenchWorkerSandbox = status;
    state.forgeBenchReferencePilot = null;
    state.forgeBenchOllamaCandidate = null;
    clearWorkstackArena(true);
    ...
    forgeBenchSandboxError = String(error || "Suppression impossible");
    setStatus(`Sandbox ForgeBench : ${forgeBenchSandboxError}`, "error");
  } finally {
    forgeBenchSandboxBusy = false;
    renderForgeBenchSandboxPanel();
...
function renderForgeBenchIsolationPanel() {
  ...
  if (ready) {
    els.forgeBenchIsolationBox.innerHTML = `
      ...
      <small>${escapeHtml(selected?.version || "version non exposée")} · aucun worker lancé par ce canari · pilote technique séparé disponible · aucune conclusion scientifique</small>
    `;
    return;
  }
...
function isForgeBenchOllamaCandidateId(value) {
  const match = String(value || "").match(/^local-model:ollama_(?:native|wsl):([A-Za-z0-9._:/-]{1,180})$/);
  const modelRef = match?.[1] || "";
  return Boolean(modelRef && !modelRef.startsWith("/") && !modelRef.includes("..") && !modelRef.includes("//"));
}

function forgeBenchLocalModelCandidates() {
  const candidates = Array.isArray(state.capabilityRouter?.candidates) ? state.capabilityRouter.candidates : [];
  return candidates
    .filter((candidate) => candidate?.available === true && candidate?.provider === "ollama" && candidate?.kind === "local_model")
    .filter((candidate) => isForgeBenchOllamaCandidateId(candidate?.id))
    .sort((left, right) => String(left.label || left.id).localeCompare(String(right.label || right.id), "fr"));
}

function forgeBenchCandidateModelRef(candidate) {
  const id = String(candidate?.id || candidate || "");
  return id.replace(/^local-model:ollama_(?:native|wsl):/, "");
}

function syncForgeBenchCandidateModelControl() {
  if (!els.forgeBenchCandidateModel) return "";
  const candidates = forgeBenchLocalModelCandidates();
  const current = els.forgeBenchCandidateModel.value;
  els.forgeBenchCandidateModel.innerHTML = candidates.length
    ? candidates.map((candidate) => `<option value="${escapeHtml(candidate.id)}">${escapeHtml(candidate.label || candidate.id)}</option>`).join("")
    : '<option value="">Aucun modèle local routé</option>';
  const ids = new Set(candidates.map((candidate) => candidate.id));
  if (ids.has(current)) {
    els.forgeBenchCandidateModel.value = current;
    return current;
  }
  const recommendedRef = recommendedModelState().ref;
  const recommended = candidates.find((candidate) => sameOllamaModel(forgeBenchCandidateModelRef(candidate), recommendedRef));
  const nonStarter = candidates.find((candidate) => !isStarterModelRef(forgeBenchCandidateModelRef(candidate)));
  const selected = recommended || nonStarter || candidates[0] || null;
  els.forgeBenchCandidateModel.value = selected?.id || "";
  return selected?.id || "";
}

function forgeBenchSelectedCandidateId() {
  const selected = syncForgeBenchCandidateModelControl();
  return forgeBenchLocalModelCandidates().some((candidate) => candidate.id === selected) ? selected : "";
}

function forgeBenchExperimentOllamaCandidate() {
  const stacks = state.forgeBench?.experiment?.candidate_stacks;
  const stack = Array.isArray(stacks) ? stacks.find((candidate) => candidate?.key === "ollama-local" && candidate?.available === true) : null;
  const binding = Array.isArray(stack?.bindings) ? stack.bindings.find((candidate) => candidate?.role === "worker") : null;
  const candidate = binding?.candidate;
  if (!candidate || !isForgeBenchOllamaCandidateId(candidate.candidate_id)) return null;
  return candidate;
}

function forgeBenchReferencePilotResult(result = state.forgeBenchReferencePilot) {
  const blockers = Array.isArray(result?.readiness?.blockers) ? result.readiness.blockers : [];
  if (
    result?.schema !== FORGEBENCH_REFERENCE_PILOT_RESULT_SCHEMA
    || !/^[a-f0-9]{64}$/i.test(String(result?.integrity?.digest || ""))
    || !["linux-bwrap-native", "wsl-bwrap"].includes(result?.selected_backend)
    || result?.consent?.confirmed !== true
    || result?.consent?.scope !== "reference_worker_pilot_v1"
    || result?.consent?.network_access !== false
    || result?.consent?.paid_api_allowed !== false
    || result?.consent?.candidate_cli_allowed !== false
    || result?.worker?.kind !== "deterministic_reference_fixture"
    || result?.worker?.started !== true
    || result?.worker?.command_executed !== true
    || result?.worker?.succeeded !== true
    || result?.worker?.timed_out !== false
    || result?.worker?.candidate_stack_executed !== false
    || result?.evaluator?.kind !== "deterministic_visible_gate"
    || result?.evaluator?.started !== true
    || result?.evaluator?.command_executed !== true
    || result?.evaluator?.succeeded !== true
    || result?.evaluator?.timed_out !== false
    ...
    || result?.evaluator?.visible_checks_total !== 6
    || result?.evaluator?.visible_checks_passed !== 6
    || !/^[a-f0-9]{64}$/i.test(String(result?.evaluator?.submission_digest || ""))
    || result?.security?.process_isolation_enforced !== true
    || result?.security?.network_namespace_enforced !== true
    || result?.security?.source_repository_mounted !== false
    || result?.security?.hidden_suite_mounted !== false
    || result?.security?.credentials_read !== false
    || result?.security?.paths_returned !== false
    || result?.security?.raw_worker_output_returned !== false
    || result?.security?.temporary_workspace_removed !== true
    || result?.readiness?.reference_runner_verified !== true
    || result?.readiness?.independent_visible_evaluator_verified !== true
    || result?.readiness?.candidate_worker_execution_ready !== false
    || result?.readiness?.scientific_eligible !== false
    || !["candidate_cli_not_executed", "hidden_suite_not_evaluated", "multi_stack_runs_not_completed", "cost_measurement_not_implemented"]
      .every((blocker) => blockers.includes(blocker))
  ) return null;
  return result;
}

function renderForgeBenchRunnerPanel() {
  if (!els.forgeBenchRunnerBox) return;
  const isolationReady = forgeBenchIsolationResult()?.readiness?.isolation_backend_ready === true;
  const workspacesReady = forgeBenchSandboxMatchesExperiment();
  const result = forgeBenchReferencePilotResult();
  els.runForgeBenchPilotBtn.disabled = !invoke || forgeBenchRunnerBusy || !isolationReady || !workspacesReady;
  if (!invoke && !result?.test_mode) {
    els.forgeBenchRunnerState.textContent = "app native requise";
    els.forgeBenchRunnerBox.className = "forgebench-runner-box empty";
    els.forgeBenchRunnerBox.textContent = "Le runner isolé est disponible uniquement dans l'application Windows/Linux.";
    return;
  }
  if (forgeBenchRunnerBusy) {
    els.forgeBenchRunnerState.textContent = "pilote en cours";
    els.forgeBenchRunnerBox.className = "forgebench-runner-box empty";
    els.forgeBenchRunnerBox.textContent = "Worker de référence isolé, puis évaluateur séparé en lecture seule. Aucun CLI IA, réseau ou test caché.";
    return;
  }
  if (forgeBenchRunnerError) {
    els.forgeBenchRunnerState.textContent = "pilote refusé";
    els.forgeBenchRunnerBox.className = "forgebench-runner-box empty";
    els.forgeBenchRunnerBox.innerHTML = `<strong>Exécution non prouvée</strong><span>${escapeHtml(forgeBenchRunnerError)}</span>`;
    return;
  }
  if (!result) {
    els.forgeBenchRunnerState.textContent = isolationReady && workspacesReady ? "prêt sur consentement" : "préflight requis";
    els.forgeBenchRunnerBox.className = "forgebench-runner-box empty";
    els.forgeBenchRunnerBox.textContent = isolationReady && workspacesReady
      ? "Lance un pilote technique sans IA pour vérifier réellement le runner et l'évaluateur avant de brancher un CLI candidat."
      : "Prépare les workspaces et vérifie le backend d'isolation avant le pilote.";
    return;
  }
  const workerMs = Number(result.worker?.duration_ms || 0);
  const evaluatorMs = Number(result.evaluator?.duration_ms || 0);
  els.forgeBenchRunnerState.textContent = "transport isolé vérifié";
  els.forgeBenchRunnerBox.className = "forgebench-runner-box";
  els.forgeBenchRunnerBox.innerHTML = `
    <strong>Worker de référence réussi · évaluateur indépendant 6/6</strong>
    <span>${escapeHtml(result.selected_backend)} · worker ${workerMs} ms · vérification ${evaluatorMs} ms · soumission montée en lecture seule</span>
    <small>Aucun Codex, Claude, Hermes ou modèle local exécuté · aucun test caché · coût API 0 € · aucune conclusion scientifique</small>
  `;
}

async function runForgeBenchReferencePilot() {
  const isolation = forgeBenchIsolationResult();
  if (!invoke || forgeBenchRunnerBusy || !forgeBenchSandboxMatchesExperiment() || isolation?.readiness?.isolation_backend_ready !== true) return null;
  if (!window.confirm("Lancer le pilote technique ForgeBench ?\n\nIl exécute un worker déterministe sans IA, sans réseau et sans API payante, puis un évaluateur séparé. Il ne teste pas encore Codex, Claude ou Hermes.")) return null;
  forgeBenchRunnerBusy = true;
  forgeBenchRunnerError = "";
  state.forgeBenchReferencePilot = null;
  state.forgeBenchOllamaCandidate = null;
  clearWorkstackArena(true);
  ...
  renderForgeBenchRunnerPanel();
  renderForgeBenchCandidatePanel();
  setStatus("Exécution du pilote isolé ForgeBench...");
  try {
    const result = await invoke("run_forgebench_reference_pilot", {
      request: {
        schema: FORGEBENCH_REFERENCE_PILOT_REQUEST_SCHEMA,
        isolation_result: isolation,
        consent: {
          confirmed: true,
          scope: "reference_worker_pilot_v1",
    ...
    if (!forgeBenchReferencePilotResult(result)) throw new Error("preuve native du pilote non conforme");
    state.forgeBenchReferencePilot = result;
    if (els.evidenceLedgerSource) els.evidenceLedgerSource.value = "forgebench_reference_pilot_verified";
    renderEvidenceLedgerPanel();
    setStatus("Pilote isolé ForgeBench vérifié", "ok");
    return result;
  } catch (error) {
    forgeBenchRunnerError = String(error || "Pilote ForgeBench impossible");
    setStatus(`Runner ForgeBench : ${forgeBenchRunnerError}`, "error");
    return null;
  } finally {
...
function forgeBenchOllamaCandidateResult(result = state.forgeBenchOllamaCandidate) {
  const blockers = Array.isArray(result?.readiness?.blockers) ? result.readiness.blockers : [];
  const experimentCandidate = forgeBenchExperimentOllamaCandidate();
  const sandbox = forgeBenchSandboxReceipt();
  const reference = forgeBenchReferencePilotResult();
  const isolation = forgeBenchIsolationResult();
  const runtime = String(result?.candidate?.runtime || "");
  const environment = runtime === "native" ? "ollama_native" : runtime === "wsl" ? "ollama_wsl" : "";
  const expectedCandidateId = `local-model:${environment}:${String(result?.candidate?.model_ref || "")}`;
  if (
    result?.schema !== FORGEBENCH_OLLAMA_CANDIDATE_RESULT_SCHEMA
    || !/^[a-f0-9]{64}$/i.test(String(result?.integrity?.digest || ""))
    ...
    || result?.candidate?.environment !== environment
    || result?.candidate?.candidate_id !== expectedCandidateId
    || result?.candidate?.model_invoked !== true
    || result?.candidate?.cli_agent_invoked !== false
    || (experimentCandidate && result.candidate.candidate_id !== experimentCandidate.candidate_id)
    || !sandbox
    || result?.batch_ref?.batch_id !== sandbox.batch_id
    || result?.batch_ref?.experiment_digest !== sandbox.experiment_digest
    || result?.batch_ref?.protocol_digest !== sandbox.protocol_digest
    || !reference
    ...
    || result?.generation?.started !== true
    || result?.generation?.succeeded !== true
    || result?.generation?.timed_out !== false
    || result?.generation?.attempts !== 1
    || !(Number(result?.generation?.duration_ms) > 0)
    || !/^[a-f0-9]{64}$/i.test(String(result?.generation?.prompt_sha256 || ""))
    || !/^[a-f0-9]{64}$/i.test(String(result?.generation?.response_sha256 || ""))
    || result?.generation?.raw_response_returned !== false
    || result?.generation?.raw_response_persisted !== false
    ...
    || result?.evaluator?.independent_process !== true
    || result?.evaluator?.workspace_read_only !== true
    || result?.evaluator?.network_namespace_enforced !== true
    || result?.evaluator?.hidden_suite_used !== false
    ...
    || result?.security?.external_network_requested !== false
    || result?.security?.loopback_ollama_requested !== true
    || result?.security?.source_repository_mounted !== false
    || result?.security?.hidden_suite_mounted !== false
    ...
    || result?.security?.temporary_workspace_removed !== true
    || result?.security?.paths_returned !== false
    || result?.cost?.api_cost_eur !== 0
    || result?.cost?.api_status !== "not_incurred"
    || result?.cost?.local_energy_wh !== null
...
function renderForgeBenchCandidatePanel() {
  if (!els.forgeBenchCandidateBox) return;
  ...
  const ready = Boolean(invoke && candidateMatches && isolationReady && workspacesReady && referenceReady);
  ...
  if (!invoke && !result?.test_mode) {
    els.forgeBenchCandidateState.textContent = "app native requise";
    els.forgeBenchCandidateBox.className = "forgebench-candidate-box empty";
    els.forgeBenchCandidateBox.textContent = "Le candidat Ollama est disponible uniquement dans l'application Windows/Linux.";
    return;
  }
  ...
  if (forgeBenchCandidateError) {
    els.forgeBenchCandidateState.textContent = "candidat refusé";
    els.forgeBenchCandidateBox.className = "forgebench-candidate-box empty";
    els.forgeBenchCandidateBox.innerHTML = `<strong>Exécution candidate non prouvée</strong><span>${escapeHtml(forgeBenchCandidateError)}</span>`;
    return;
  }
  if (result) {
  ...
  els.forgeBenchCandidateState.textContent = ready ? "prêt sur consentement" : "préflight requis";
  els.forgeBenchCandidateBox.className = "forgebench-candidate-box empty";
  if (!selectedId) {
    els.forgeBenchCandidateBox.textContent = "Aucun modèle Ollama installé n'a été routé. Relance le scan puis Capability Router.";
  } else if (!candidate) {
    els.forgeBenchCandidateBox.textContent = "Coche Modèle Ollama local, puis prépare à nouveau l'expérience et les workspaces.";
  } else if (!candidateMatches) {
    els.forgeBenchCandidateBox.textContent = "Le modèle choisi a changé. Recompile l'expérience pour signer cette identité exacte.";
...
async function runForgeBenchOllamaCandidate() {
  ...
  const durationSeconds = Number(els.forgeBenchCandidateDuration?.value || 300);
  if (
    !invoke
    || forgeBenchCandidateBusy
    || !state.forgeBench
    || !candidate
  ...
  state.forgeBenchOllamaCandidate = null;
  renderForgeBenchCandidatePanel();
  setStatus(`ForgeBench interroge ${modelRef} via Ollama local...`);
  try {
    const result = await invoke("run_forgebench_ollama_candidate", {
      request: {
        schema: FORGEBENCH_OLLAMA_CANDIDATE_REQUEST_SCHEMA,
        forgebench_result: state.forgeBench,
        reference_pilot_result: reference,
        isolation_result: isolation,
        consent: {
          confirmed: true,
    ...
    if (!forgeBenchOllamaCandidateResult(result)) throw new Error("preuve native du candidat Ollama non conforme");
    ...
    setStatus(`Candidat ForgeBench : ${forgeBenchCandidateError}`, "error");
    return null;
  } finally {
    forgeBenchCandidateBusy = false;
    renderForgeBenchCandidatePanel();
  }
}
...
function forgeBenchMarkdown(result = state.forgeBench) {
  ...
  const localCandidate = forgeBenchOllamaCandidateResult();
  return [
    ...
    `- Candidat Ollama local : ${localCandidate ? `${localCandidate.candidate.model_ref} généré, structure 7/7, code non exécuté` : "non exécuté"}`,
    ...
    `- Agent CLI candidat lancé : non`,
    `- Modèle local candidat lancé : ${localCandidate ? "oui" : "non"}`,
    `- Scores calculés : non`,
...
function renderForgeBenchPanel() {
  ...
  const selectedCandidateId = syncForgeBenchCandidateModelControl();
  const localStackReady = !selectedStacks.includes("ollama-local") || Boolean(selectedCandidateId);
  const nativeOrTest = Boolean(invoke || result?.test_mode);
  els.compileForgeBenchBtn.disabled = !invoke || forgeBenchBusy || !plan || !router || selectedStacks.length < 2 || !localStackReady;
  ...
  if (!experiment) {
    els.forgeBenchState.textContent = selectedStacks.length < 2 ? "deux stacks requises" : localStackReady ? "prêt à préparer" : "modèle local requis";
    els.forgeBenchBox.className = "forgebench-box empty";
    els.forgeBenchBox.textContent = selectedStacks.length >= 2 && localStackReady
      ...
      : selectedStacks.length < 2
        ...
        : "Sélectionne un modèle Ollama installé pour signer la stack locale.";
    return;
  }
  ...
  els.forgeBenchBox.innerHTML = `
    ...
    ${protocol.hidden_suite?.status === "locally_sealed"
      ? '<div class="forgebench-warning">Suite privée non montée dans le pilote et pas encore isolée pour les futurs agents : aucune conclusion scientifique autorisée.</div>'
...
async function compileForgeBenchExperiment() {
  ...
  const ollamaCandidateId = stacks.includes("ollama-local") ? forgeBenchSelectedCandidateId() : "";
  if (!invoke || forgeBenchBusy || !plan || !router || stacks.length < 2 || (stacks.includes("ollama-local") && !ollamaCandidateId)) return null;
  forgeBenchBusy = true;
  forgeBenchError = "";
  state.forgeBench = null;
  state.forgeBenchReferencePilot = null;
  ...
  try {
    const result = await invoke("compile_forgebench_experiment", {
      request: {
        ...
        ollama_candidate_id: ollamaCandidateId || null
      }
    });
...
function clearForgeBench(silent = false) {
  ...
  if (!silent) {
    ...
    for (const input of els.forgeBenchStacks?.querySelectorAll('input[name="forgeBenchStack"]') || []) input.checked = input.value !== "ollama-local";
  }
  renderForgeBenchPanel();
  renderForgeBenchSandboxPanel();
  renderForgeBenchRunnerPanel();
...
function installTestHarness() {
  ...
  window.__OUTILSIA_TEST__ = {
    ...
    applyForgeBenchState() {
      ...
      const binding = (role, candidateId, label, environment = "test") => ({
        role,
        candidate: { candidate_id: candidateId, label, environment, version: "1.0.0", auth_status: "not_inspected", quota_verified: false },
        execution_started: false
      });
      state.forgeBench = {
      ...
      state.forgeBenchWorkerSandbox = {
        ...
        test_mode: true,
        receipt: {
          schema: FORGEBENCH_WORKER_SANDBOX_RECEIPT_SCHEMA,
          batch_id: "fbsb-demo-signal-maze",
          benchmark_id: "signal-maze-v1",
          experiment_id: "fb-demo-signal-maze",
          experiment_digest: "f".repeat(64),
          protocol_digest: protocolDigest,
          candidate_stacks_total: 4,
          public_seeds_total: 3,
          workspaces_total: 12,
          starter_bundle_sha256: "c".repeat(64),
          batch_digest: "2".repeat(64),
          paths_returned: false,
          security: {
          ...
          execution: { worker_started: false, command_executed: false, network_called: false, repository_modified: false, api_spend_eur: 0 },
          readiness: {
            workspace_batch_prepared: true,
            worker_execution_ready: false,
            scientific_eligible: false,
            blockers: ["worker_process_not_started", "process_isolation_not_enforced", "network_isolation_not_enforced", "hidden_suite_not_worker_inaccessible", "isolated_evaluator_not_implemented"]
          },
      ...
      state.forgeBenchReferencePilot = {
        ...
        contract_version: "2026-07-13",
        pilot_id: "fbp-demo-reference-run",
        benchmark: { id: "signal-maze-v1", track: "transport_and_isolation_pilot" },
        batch_ref: { batch_id: "fbsb-demo-signal-maze", experiment_digest: "f".repeat(64), protocol_digest: protocolDigest, public_seed_sha256: "5".repeat(64) },
        host_environment: "linux",
        selected_backend: "linux-bwrap-native",
        consent: { confirmed: true, scope: "reference_worker_pilot_v1", network_access: false, paid_api_allowed: false, candidate_cli_allowed: false },
        worker: { kind: "deterministic_reference_fixture", started: true, command_executed: true, succeeded: true, timed_out: false, duration_ms: 184, stdout_marker_verified: true, candidate_stack_executed: false, output_files_total: 6, output_bytes: 3184 },
        evaluator: { kind: "deterministic_visible_gate", started: true, command_executed: true, succeeded: true, timed_out: false, duration_ms: 126, independent_process: true, workspace_read_only: true, hidden_suite_used: false, visible_checks_total: 6, visible_checks_passed: 6, submission_digest: "6".repeat(64) },
        security: { process_isolation_enforced: true, mount_namespace_enforced: true, network_namespace_enforced: true, pid_namespace_enforced: true, host_root_hidden: true, source_repository_mounted: false, hidden_suite_mounted: false, credentials_read: false, paths_returned: false, raw_worker_output_returned: false, temporary_workspace_removed: true, host_sentinel_unchanged: true },
        cost: { api_cost_eur: 0, status: "not_incurred" },
        readiness: { reference_runner_verified: true, independent_visible_evaluator_verified: true, candidate_worker_execution_ready: false, scientific_eligible: false, blockers: ["candidate_cli_not_executed", "hidden_suite_not_evaluated", "multi_stack_runs_not_completed", "cost_measurement_not_implemented"] },
        integrity: { digest: "7".repeat(64) },
        test_mode: true
      };
      state.forgeBenchOllamaCandidate = {
...
els.forgeBenchCandidateModel?.addEventListener("change", () => {
  state.forgeBench = null;
  state.forgeBenchReferencePilot = null;
  state.forgeBenchOllamaCandidate = null;
  clearWorkstackArena(true);
...
els.forgeBenchCandidateDuration?.addEventListener("change", () => {
  state.forgeBenchOllamaCandidate = null;
  forgeBenchCandidateError = "";
  renderForgeBenchCandidatePanel();
  renderEvidenceLedgerPanel();
