...
try {
  ...
  if (worker.status !== 0 || !worker.stdout.includes("worker_marker=forgebench-reference-worker-ok")) {
    throw new Error(`reference worker failed status=${worker.status} signal=${worker.signal || "none"}`);
  }
  ...
  if (evaluator.status !== 0 || !evaluator.stdout.includes("evaluator_marker=forgebench-visible-evaluator-ok")) {
    throw new Error(`visible evaluator failed status=${evaluator.status} signal=${evaluator.signal || "none"}`);
  }
  const expectedFiles = [
