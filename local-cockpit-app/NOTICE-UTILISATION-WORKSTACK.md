...
| Benchmark Commons v1 | Préparer après une mesure standard un JSON pseudonymisé, le vérifier avant export et permettre son retrait local. | Aucun upload, synchronisation, preuve terrain, validation communautaire ou classement. |
...
**Contrôle recommandé :** déplier **Ce qui peut réellement s'exécuter** sous le Router. Le registre `outilsia.agent_adapter_policy_catalog.v1` doit annoncer un seul pilote borné, Codex CLI sur `Signal Maze v1`, et trois adaptateurs en détection seule, Claude Code, Hermes Agent et Kimi Code. Le registre ne sonde aucun compte, quota, credential, dépôt ou board et ne vaut jamais consentement de run.
