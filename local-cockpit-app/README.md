...
npm run test:release-update-policy
npm run verify:release-update-ui
```
...
ForgeBench v0 prépare désormais `Signal Maze v1` dans l'espace Atelier IA : starter public scellé, trois seeds visibles, mêmes règles et budgets pour chaque stack, score futur explicite et readiness exploratoire/scientifique séparée. Le préflight seul n'exécute aucun agent ; le holdout local exige une suite privée scellée et un consentement de run distinct.
...
Le **Préflight Chromium** lance ensuite une page minimale dans le même runtime isolé, sans réseau ni code candidat. Il distingue une installation système d'un cache Playwright, ne retourne aucun chemin et ne lance aucune installation. Si Chromium manque, OutilsIA propose seulement une commande Playwright bornée à copier lorsque `npx` ou Playwright Python est déjà disponible ; l'utilisateur reste responsable de l'exécuter et d'accepter le réseau ou l'élévation éventuelle.
...
Le **Candidat Ollama local** est le premier adaptateur de modèle réellement exécutable dans les sources. Après un second consentement, un modèle déjà installé reçoit seulement la tâche, le starter et le `Visible Gameplay Contract v1` publics via l'API de boucle locale. Sa réponse stricte produit trois fichiers bornés, contrôlés puis joués dans Chromium sous bubblewrap, sans réseau et avec la soumission en lecture seule. Après gel de ces fichiers, un second Chromium isolé applique cinq familles de holdout sur les seeds du vault. Aucun seed, identifiant de check, observation privée, DOM ou capture du holdout n'est retourné. La sortie brute n'est ni retournée ni persistée ; le Ledger conserve seulement métriques et SHA-256. Le holdout prouve ce run borné, pas une supériorité scientifique : familles de checks publiques, vault non chiffré, isolation même-utilisateur, pairs et énergie restent des blocages explicites. Aucun CLI Codex, Claude ou Hermes n'est lancé par ce palier.

Le **pilote Workstack Arena Codex** est le premier adaptateur CLI réellement exécutable dans les sources. Il ne traite pas encore une carte ou un dépôt : il accepte uniquement la stack `codex-solo` sur `Signal Maze v1` public, dans une copie jetable du workspace ForgeBench. Deux consentements par run, une tentative, un budget exact de 3, 5 ou 10 minutes et un plafond de 512 Kio encadrent l'invocation. Les règles externes sont ignorées et une allowlist d'environnement exclut les clés API tierces, tokens cloud et socket SSH. OutilsIA ne transmet et ne monte ni dépôt utilisateur, board, suite cachée ou credential ; le mode `workspace-write` reste néanmoins une propriété de la sandbox Codex, pas une preuve indépendante d'isolation de lecture de l'hôte. La sortie brute est supprimée, les trois fichiers passent `7/7` checks statiques puis `39/39` checks Chromium publics, le coût ou quota reste inconnu et toute livraison attend une revue humaine. Claude Code, Hermes, Kimi, l'exécution arbitraire et le classement scientifique restent hors de ce palier non publié.

Le **registre Agent Adapter Policy v1** rend cette frontière lisible sous Capability Router. Sa commande Tauri retourne un catalogue local signé et n'exécute rien : Codex CLI possède uniquement le pilote public borné ci-dessus ; Claude Code, Hermes Agent et Kimi Code sont `detect_only`, sans scope, budget ou workspace autorisé. La sonde Kimi se limite à `kimi --version` et ne lit ni compte, jeton, quota ou configuration. Détection, autorisation et consentement restent trois états distincts.

La **revue humaine du reçu** ferme maintenant cette boucle sans surpromesse. Le propriétaire accepte le reçu pour une future comparaison, demande un nouveau run corrigé ou rejette le run. Le résultat signé est lié à l'empreinte Workstack Arena et peut rejoindre une seule fois l'Evidence Ledger. Comme le code et les captures ne sont pas conservés, la revue ne revendique jamais une inspection visuelle ou une approbation de qualité et n'autorise ni livraison, gagnant, écriture board, fusion ou publication.
