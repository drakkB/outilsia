...
- **ForgeBench Ollama Candidate v0 (source candidate, not public yet)** lets one explicitly selected, already-installed local model propose the three files of the public Signal Maze starter. The model receives no private files, tools, credentials, hidden suite or Internet access; the host materializes only `index.html`, `styles.css` and `game.js`, then a separate Bubblewrap evaluator checks structure and isolation without executing the generated code. A `7/7` receipt proves only static admissibility, not gameplay quality, benchmark performance, scientific validity or a winner.
