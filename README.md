...
- **ForgeBench Garden/Bamboo v1 (source candidate, not public yet)** compares 1–8 bounded GardenScript policies on one frozen synthetic 365-day contract and an identical sealed suite read only after all sources are frozen. The deterministic interpreter runs no arbitrary candidate code, process or network. Its lexicographic order has no composite score; speed, cost and energy stay separate. It is not an official GardenArena ranking, scientific result, horticultural advice or declared winner.
- **Workstack Arena Codex pilot (source candidate, not public yet)** invokes the exact routed Codex CLI once on that same public Signal Maze task, after separate quota/cost and execution consents. It uses a disposable ForgeBench workspace, bounded time and output, ignores external rules, passes only an allowlisted environment without third-party API keys, persists no raw CLI response, reports vendor cost as unknown and requires human review. OutilsIA does not transmit or mount a user repository, board, hidden suite or credential; arbitrary project work, Claude Code, Hermes, hidden evaluation and winner selection remain unavailable.
