...
- **Private Workload Packs v1 (cross-platform candidate `291887472771`, not yet claimed as public)** compares one identical Code, French, summary, Memory/Obsidian or custom business task on two or three already-installed models. It downloads nothing, uploads nothing, persists no raw prompt/output and exports only scores, checks and SHA-256 digests to report, PDF, MemoryForge and Capability Passport 1.3.0.
- **ForgeBench Ollama Browser v1 (source candidate, not public yet)** lets one explicitly selected, already-installed local model propose the three files of the public Signal Maze starter. The model receives no private files, tools, credentials, hidden suite or Internet access; the host materializes only `index.html`, `styles.css` and `game.js`, then separate Bubblewrap processes run `7/7` static checks and `39/39` public Chromium gameplay checks across desktop and Android inputs. This proves only the visible public run, not hidden quality, scientific validity or a winner.
- **Workstack Arena Codex pilot (source candidate, not public yet)** invokes the exact routed Codex CLI once on that same public Signal Maze task, after separate quota/cost and execution consents. It uses a disposable ForgeBench workspace, bounded time and output, ignores external rules, passes only an allowlisted environment without third-party API keys, persists no raw CLI response, reports vendor cost as unknown and requires human review. OutilsIA does not transmit or mount a user repository, board, hidden suite or credential; arbitrary project work, Claude Code, Hermes, Kimi, hidden evaluation and winner selection remain unavailable.
- **Agent Adapter Policy v1 (source candidate, not public yet)** exposes a signed, read-only registry below Capability Router. Codex CLI is limited to the public Signal Maze pilot; Claude Code, Hermes Agent and Kimi Code remain detection-only with no authorized scope, budget or workspace. Reading the registry never launches an agent or inspects an account, quota, credential, repository or board.
- **Signed human receipt review (source candidate, not public yet)** binds one structured local decision to the exact Workstack Arena digest: accept for future comparison, request a corrected rerun or reject the run. It stores no free text and never claims the deleted code or screenshots were inspected. Delivery, winner selection, board writes, merge and publication remain forbidden for every decision.
...
- **AI Capability Passport v1.3** exports a versioned JSON document containing hardware evidence, runtimes, installed models, benchmarks, recommendation, the sanitized installation preflight and the read-only Strategy Arena handoff boundary.
- **Install Safety Preflight v1 (source candidate, not public yet)** checks the selected native or WSL Ollama runtime and its model-store volume before every explicit pull. A measured shortage blocks before download; unknown storage requires confirmation; personal storage paths never enter reports or Passport exports.
...
- **Local MCP v0.1 (source candidate, not public yet)** exposes the same frozen evidence through MCP Streamable HTTP with eight read-only tools and four resources. Native black-box checks cover authentication, expiry and cleanup, and bounded client recipes exercise Codex CLI and Claude Code. A future action lane will remain a separate contract where only a native human confirmation can authorize one exact, short-lived plan; model-supplied confirmation is never accepted.
